...
WHERE `category`= '$filde' ";
    ...
    public function andget_total($filde,$_and){
       //$sql="SELECT {$filde} FROM {$table}";
       $sql="SELECT `organizetions`.`*`, `address`.`*`,`locations`.`*`,`services`.`*`
FROM `organizetions`
INNER JOIN `address` ON 
...
WHERE `category`= '$filde' ";
      if($_and!=""){
           $sql .= "AND `city`='$_and' ORDER BY `category` ASC ";
       } else {
          $sql .="ORDER BY `category` ASC ";
      }
...
//
public function neartogat_total($category,$lat,$lng){
     $sql="SELECT `organizetions`.`*`, `address`.`*`,`locations`.`*`,`services`.`*`,( 
6371 * acos(
 cos( radians($lat) ) 
 * cos( radians( latitude ) ) 
...
public function updateAdmin($tabel, $id, $fileds=array()){
  ...
  //UPDATE `organizetions` SET `account_status` = '0' WHERE `id` ='53'
     ...
     foreach ($fileds as $name=>$value){
      //change by abel:to recive a key Array and concatnating set value
         $set .="{$name}"."="." {$value}";
         /*if($x < count($fileds)){
             $set .=', ';
         }*/
    ...
    //return $set;//account_status= deactivate
    // die($set); username= ?,email= ?
    //$sql = "UPDATE users SET usermane= newusername WHERE id=2";
